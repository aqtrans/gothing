...
func Diff(file, hash string) ([]byte, error) {
	...
	git := exec.Command("git", "-C", "data", "show", "--oneline", "--no-color", hash, file)
...
func Commits(filename string, n int) ([]Commit, error) {
	...
	cmd := exec.Command("git", "-C", "data", "log", "-n", strconv.Itoa(n), logFormat, filename)
...
func IsGitRepository(path string) bool {
	var out bytes.Buffer
	cmd := exec.Command("git", "-C", "data", "rev-parse", "--is-inside-work-tree")
	cmd.Stdout = &out
